...
2. Run the following command to compile your workflow:
   ```bash
   pixi run compile-nest-monitoring
   ```
   
   This will generate a folder called `ecoscope-workflows-nest-monitoring-workflow` with your compiled workflow.

   Later on if you update the workflow spec you can recompile it using:
   ```bash
   pixi run recompile-nest-monitoring
   ```
