...
@task
def replace_missing_with_label(df: AnyDataFrame, columns: str | list[str], label: str) -> AnyDataFrame:
    """
    ...
    if missing_cols:
        raise ValueError(f"Column(s) {missing_cols} not found. " f"Available columns: {list(df.columns)}")
...
def to_sentence_case(df: AnyDataFrame, columns: str | list[str]) -> AnyDataFrame:
    ...
    missing_cols = [col for col in columns if col not in df.columns]
    if missing_cols:
        raise ValueError(f"Column(s) {missing_cols} not found in DataFrame. " f"Available columns: {list(df.columns)}")
...
@task
def bin_columns(
    df: AnyDataFrame, columns: Union[str, List[str]], bins: int = 5, suffix: str = " bins", inplace: bool = False
) -> AnyDataFrame:
    """
...
@task
def pivot_df(df: AnyDataFrame, index_col: str, columns_col=str, values_col=str, reset_idx=True) -> AnyDataFrame:
...
def explode_multiple_columns(
    df: AnyDataFrame, columns: Union[str, List[str]], reset_index: bool = True
) -> AnyDataFrame:
    """
