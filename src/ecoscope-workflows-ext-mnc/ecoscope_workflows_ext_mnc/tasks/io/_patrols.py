...
def create_patrol_coverage_grid(trajs: AnyGeoDataFrame, grid_cell_size=1000) -> AnyGeoDataFrame:
    ...
    trajectories_for_overlay = trajectories_for_overlay[
        ["original_seg_id", "timespan_seconds", "dist_meters", "patrol_id", "geometry"]
    ]
    ...
    clipped_trajs["clipped_timespan_seconds"] = clipped_trajs["timespan_seconds"] * (
    ...
    full_grid_summary["distance_patrolled_km"] = (full_grid_summary["distance_patrolled_meters"] / 1000).round(2)
    full_grid_summary["time_spent_hours"] = (full_grid_summary["time_spent_seconds"] / 3600).round(2)
...
def get_patrol_values(
    ...
    def fetch(patrol_id):
        try:
            ...
            if not data:
                print(f"No data returned for patrol: {patrol_id}")
            return data
        except Exception as e:
            print(f"Error fetching patrol {patrol_id}: {e}")
            return None
