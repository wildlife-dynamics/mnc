...
class TestMergeMultipleDf:
    ...
    def sample_df3(self):
        ...
        return pd.DataFrame({"id": [5, 6], "value": [50, 60]})

    @pytest.fixture
    def sample_gdf1(self):
        """Create first sample GeoDataFrame"""
        return gpd.GeoDataFrame(
            {"id": [1, 2], "name": ["A", "B"]}, geometry=[Point(0, 0), Point(1, 1)], crs="EPSG:4326"
        )
    ...
    def sample_gdf2(self):
        ...
        return gpd.GeoDataFrame(
            {"id": [3, 4], "name": ["C", "D"]}, geometry=[Point(2, 2), Point(3, 3)], crs="EPSG:4326"
        )
    ...
    def test_merge_with_multiple_skip_sentinels(self, sample_df1, sample_df2):
        ...
        result = merge_multiple_df([SKIP_SENTINEL, sample_df1, SKIP_SENTINEL, sample_df2, SKIP_SENTINEL])
        assert len(result) == 4
        assert result["id"].tolist() == [1, 2, 3, 4]
